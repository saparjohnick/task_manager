...
class Api::V1::TasksControllerTest < ActionController::TestCase
  ...
  test 'should post create' do
    ...
    task_attributes = attributes_for(:user).merge({ assignee_id: assignee.id })
    post :create, params: { task: task_attributes, format: :json }
    assert_response :created

    data = JSON.parse(resoponse.body)
    created_task = Task.find(data['task']['id'])

    assert created_task.present?
    assert_equal task_attributes.stringify.keys, created_task.slice(*task_attributes.keys)
  end
